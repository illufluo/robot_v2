    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Detect objects ONLY for current state (performance optimization)
                blocks = []
                sheets = []
                
                if self.current_state == RobotState.FIND_BLOCK:
                    # Only detect blocks when finding blocks
                    blocks = self.vision.detect_small_blocks(frame)
                elif self.current_state == RobotState.ALIGN_TO_TARGET_SHEET:
                    # Only detect target color sheet when aligning
                    if self.current_block_color:
                        sheets = self.vision.detect_sheets(frame, colors=[self.current_block_color])
                # Other states don't need detection
                
                # Draw detections (lightweight)
